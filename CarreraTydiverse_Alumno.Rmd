...
cran %>%
  subset(select = c('ip_id', 'package', 'country'))
...
by_package %>% 
  summarise(size.media = mean(size))
...
top_counts <- cran %>% 
  group_by(package) %>%
  summarise(num_descargas = n()) %>%
  filter(num_descargas > quantile(.$num_descargas, probs = 0.99)) %>%
  arrange(desc(num_descargas)) %>%
  select("package")

# Almacenos solo los paquetes porque es lo que dice el enunciado
top_counts
...
# Como ya ordené en el ejercicio anterior simplemente asigno la variable 
# anterior a esta
...
top_unique <- cran %>% 
  group_by(package) %>%
  summarise(ordenadores = unique(ip_id)) %>%
  filter(ordenadores > quantile(.$ordenadores, probs = 0.99)) %>%
  arrange(desc(ordenadores)) %>%
  select("package")
...
cran %>% 
  group_by(country) %>%
  summarise(num_descargas = n()) %>%
  filter(num_descargas > quantile(.$num_descargas, probs = 0.9)) %>%
  arrange(desc(num_descargas)) %>%
  ggplot(aes(x = country, y = num_descargas)) + 
    geom_point()
...
# Variables: 
# grade : Factor w/ 5 levels "A","B","C","D",..: 1 2 3 4 5
# male  : int  1 5 5 5 7
# female: int  5 0 2 5 4

students %>% 
  pivot_longer(names_to = "sex", values_to = "count", cols = c("male","female"))
